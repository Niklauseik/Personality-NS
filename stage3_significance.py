...
RESERVED_DIRS = {"base", "summaries", "plots", "meta", "stage3"}
NUMERIC_COLS = [
    "statistic",
    "df",
    "p_value",
...
def _write_plot_readme_root(root_dir: Path, model_roots: list[Path], min_p: float, p_threshold: float) -> None:
    readme_path = root_dir / "STAGE3_README.md"
    model_list = "\n".join([f"- `{p.name}/stage3/`" for p in model_roots]) if model_roots else "- (无输出)"
    content = f"""# Stage-3 绘图说明
...
- 横轴：数据集
- 纵轴：性格维度
- 颜色：`-log10(p)`，p-value 使用下限 {min_p}，避免出现 0
- 参考：`p < {p_threshold}` 对应 `-log10(p) > {-math.log10(p_threshold):.2f}`
...
def main() -> None:
    ...
    if args.dry_run:
        print("[Stage-3] Dry-run. Would process:")
    ...
    for target in targets:
        if not _is_model_root(target):
            print(f"[Stage-3] Skipping non-newlayout root: {target}")
            continue
        long_df = _build_long_table(target)
        if long_df.empty:
            print(f"[Stage-3] No significance CSV found under: {target}")
            continue
        _write_outputs(target, long_df, min_p=args.min_p, p_threshold=args.p_threshold)
        processed.append(target)
        print(f"[Stage-3] Completed: {target}")
