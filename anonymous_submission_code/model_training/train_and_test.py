...
Examples:
  python run_model_training.py --dimension information --model-path ./llama-3B-Instruct
  python run_model_training.py --pair ENTP ISFJ --model-path ./llama-3B-Instruct
  python run_model_training.py --pair ENTP ISFJ --model-path ./llama-3B-Instruct --no-benchmark
  python run_model_training.py --dimension decision --model-path ./llama-3B-Instruct --sentiment-runs 3
"""
import re
import shutil
...
def run(args) -> None:
    ...
    print("\n[Model training] Results will be stored under:")
    for root in results_roots:
        print(f"  - {root}")

    if args.dimension:
        print("\n[Model training] Training personality subtypes...")
        ...
        print(f"\n[Model training] Training custom personality pair: {pair_display}")
    ...
    if args.benchmark:
        print("\n[Model training] Running benchmark evaluations...")
        ...
        if len(results_roots) > 1:
            ...
            if src.exists():
                ...
                print(f"[Model training] Benchmark output not found at {src}; skipping copy to other runs.")
    else:
        print("\n[Model training] Benchmark is disabled; skipping benchmark evaluations.")
    for run_idx, results_root in enumerate(results_roots, start=1):
        banner = f" (run {run_idx}/{len(results_roots)})" if len(results_roots) > 1 else ""
        print(f"\n[Model training] Running sentiment inference{banner}...")
        run_sentiment(sentiment_model_specs, results_root=results_root, file_suffix="")

    print("\n[Model training] Completed. Metadata saved to:")
