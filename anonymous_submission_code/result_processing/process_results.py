# -*- coding: utf-8 -*-
"""
Result processing pipeline: post-process sentiment outputs, evaluate, and draw charts.

Examples:
  python process_results.py --results-root results-NS-first-run
  python process_results.py --results-root results-NS-first-run results-ST-NF-first-run
  python process_results.py --results-glob "results-*" --continue-on-error

New layout (model root, no pipeline_state.json, ASCII-only paths):
  python process_results.py --results-root llama-3b_newlayout
"""
...
def _run_mbti_for_model_specs(model_specs: list[dict], output_dir: Path, args, context: str) -> None:
    if bool(getattr(args, "skip_mbti", False)):
        print(f"\n[Result processing] Skipping MBTI validation for {context} (--skip-mbti).")
        return
    if not model_specs:
        print(f"\n[Result processing] No checkpoint-like model paths found for MBTI validation in {context}; skipping.")
        return

    print(f"\n[Result processing] Running MBTI personality validation for {context}...")
...
def _run_processing_for_root(results_root: Path, args=None) -> None:
    print(f"\n========== [Result processing] Processing: {results_root} ==========")
    if args is not None:
    ...
    if benchmark_root is not None:
        ...
        print(
            "[Result processing] Benchmark results not found under "
            f"{results_root / 'benchmark'} (or parent folders); skipping."
        )
    print("\n[Result processing] Generating visualizations...")
    from .draw_charts import generate_charts

    generate_charts(results_root, chart_data)
    print("\n[Result processing] Completed all processing steps.")


def _run_processing_for_newlayout_model_root(model_root: Path, args=None) -> None:
    print(f"\n========== [Result processing] Processing model root (new layout): {model_root} ==========")
    if args is not None:
        _run_mbti_for_model_specs(
    ...
    print("\n[Result processing] Completed new-layout processing.")
...
def run(args) -> None:
    ...
    if args.dry_run:
        print("[Result processing] Dry-run. Would process:")
    ...
    for root in newlayout_model_roots:
        try:
            _run_processing_for_newlayout_model_root(root, args=args)
        except Exception as exc:
            failures += 1
            print(f"\n[Result processing] ERROR while processing new layout {root}: {exc}")
    ...
    for root in legacy_run_roots:
        ...
        try:
            _run_processing_for_root(root, args=args)
        except Exception as exc:
            failures += 1
            print(f"\n[Result processing] ERROR while processing legacy run root {root}: {exc}")
            if not args.continue_on_error:
                raise
    if failures:
        raise SystemExit(f"[Result processing] Completed with failures: {failures}")
