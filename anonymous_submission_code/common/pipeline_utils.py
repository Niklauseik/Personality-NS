...
and convenience utilities so the training and processing modules can stay
in sync.
"""
...
def load_pipeline_state(results_root: Path) -> Dict:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Please run model training and inference first."
        )
...
def ordered_sentiment_entries(results_root: Path) -> List[Dict]:
    ...
    When model training is run with --no-base-sentiment, the pipeline metadata still
