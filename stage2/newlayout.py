...
def process_model_root(model_root: Path) -> None:
    ...
    if not dimensions:
        print(f"[Stage-2] No dimensions/pairs found under: {model_root}")
        return
...
def _process_pair_root(pair_root: Path, base_root: Path) -> None:
    ...
    if len(model_codes) < 2:
        print(f"[Stage-2] Skipping {pair_root}: expected 2 model folders, found {len(model_codes)}.")
        return
    if len(model_codes) > 2:
        print(f"[Stage-2] WARNING: {pair_root} has {len(model_codes)} model folders; using first 2: {model_codes[:2]}")
    ...
    if not runs:
        print(f"[Stage-2] Skipping {pair_root}: no shared sentiment runs found under both models.")
        return
    ...
    if not datasets:
        print(f"[Stage-2] No known sentiment datasets found under: {pair_root}")
        return
    ...
    for dataset_key in datasets:
        ...
        if not base_csv.exists():
            print(f"[Stage-2] WARNING: missing base sentiment file: {base_csv}")
            continue
        ...
        for run_name in runs:
            ...
            for code in model_codes:
                ...
                if not model_csv.exists():
                    print(f"[Stage-2] WARNING: missing sentiment file: {model_csv}")
                    missing = True
                    continue
